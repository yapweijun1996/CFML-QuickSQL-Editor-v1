...
<cfif form.tableName EQ "" OR form.rowId EQ "" OR form.column EQ "" OR form.pkName EQ "">
  <cfoutput>{"status":"error","message":"Missing parameters"}</cfoutput>
  <cfexit>
</cfif>
...
<cfif NOT listFindNoCase(arrayToList(allowedTables), form.tableName)>
  <cfoutput>{"status":"error","message":"Invalid table name"}</cfoutput>
  <cfexit>
</cfif>
...
<cfif NOT listFindNoCase(arrayToList(allowedColumns), form.column)>
  <cfoutput>{"status":"error","message":"Invalid column name"}</cfoutput>
  <cfexit>
</cfif>
